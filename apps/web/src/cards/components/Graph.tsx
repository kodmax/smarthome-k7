...
export const Graph: FC<GraphProps> = ({ data, scaleY, scaleX, valueKey = 'value', activeKey = 'active' }) => {
    if (data) {
        ...
        const min = Math.min(...values); const max = Math.max(...values); const mid = (min + max) / 2
        ...
        const points = records.filter(({ age }) => age <= width).map(({ age, value }) => {
            const y = Number(height / 2 - (value - mid) / stepY).toFixed(4)
            const x = Number(width - age).toFixed(4)
            return `${x},${y}`
        })

        const actives = records.filter(record => record.age <= width && record.active).map(({ age }) => {
            return Number(width - age).toFixed(4)
        })

        const svg = renderToString(
            <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`} strokeWidth={5} version='1.1' xmlns='http://www.w3.org/2000/svg'>
                <>
                    <polyline stroke={spread > scaleY ? 'hsl(0deg 100% 50%)' : 'hsl(0deg 0% 20%)'} fill='none' points={points.join(' ')} />
                    {actives.map(x => <circle key={x} cx={x} cy='98' r='0.2' strokeWidth='0' fill='red' />)}
                </>
            </svg>
        )
