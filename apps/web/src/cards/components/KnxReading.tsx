...
const KnxReading: FC<Props> = ({ icon, id, label, unit = true, range, onUpdate, bars, graph, precision = 0, target }) => {
    ...
    if (reading) {
        return (<tr>
            ...
            <td>
                ...
                { !graph ? null : <Graph scaleX={graph.scaleX} scaleY={graph.scaleY} valueKey={graph.valueKey} data={reading[graph.historyKey ?? 'value'] as DataPoint[]} />}
            </td>
            <td>
                <span>{icon}</span>
