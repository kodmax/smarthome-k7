import { type Translations } from './types'

export const ru: Translations = {
  nav: {
    dashboard: 'Главная',
    stockMarket: 'Биржа',
    jobMarket: 'Рынок труда',
    energyMeter: 'Учёт энергии',
    appearance: 'Внешний вид',
    about: 'О приложении',
    settings: 'Настройки',
  },
  appearance: {
    title: 'Внешний вид',
    description: 'Настройки внешнего вида приложения.',
    theme: 'Тема',
    themeAriaLabel: 'Тема приложения',
    system: 'Системная',
    systemAriaLabel: 'Системная тема',
    light: 'Светлая',
    lightAriaLabel: 'Светлая тема',
    dark: 'Тёмная',
    darkAriaLabel: 'Тёмная тема',
    language: 'Язык',
    languageAriaLabel: 'Язык приложения',
    sentryTest: 'Sentry',
    sentryTestDescription: 'Отправить тестовую ошибку на frontend и backend для проверки source maps в Sentry.',
    sentryTestButton: 'Отправить тестовую ошибку',
    sentryTestSent: 'Тестовые ошибки отправлены в Sentry (web и service).',
    sentryTestUnavailable: 'Sentry доступен только в production при настроенном DSN.',
  },
  stockMarket: {
    title: 'Биржа',
    description: 'Биржевые котировки и анализ компаний',
    marketStatus: {
      title: 'Сессия биржи',
    },
    marketIndices: {
      sp500: {
        title: 'S&P 500',
      },
      sp500Futures: {
        title: 'Фьючерсы',
      },
    },
    forex: {
      usdPln: {
        title: 'Курс USD/PLN',
      },
    },
    earningsTomorrow: {
      title: 'Квартальный отчёт завтра',
    },
    highUpside: {
      title: 'Большой upside',
    },
    lowForwardPE: {
      title: 'Низкий прогнозный P/E на цели',
    },
    earningsSoon: {
      title: 'Квартальный отчёт скоро',
    },
    priceTargetChange: {
      title: 'Изменение цели',
    },
    quotesOverview: {
      title: 'Обзор',
    },
    views: {
      label: 'Вид',
    },
  },
  stockQuotes: {
    title: 'Котировки',
  },
  jobMarket: {
    title: 'Рынок труда',
    description: 'Анализ рынка вакансий — JavaScript / TypeScript',
    summary: {
      activeOffers: 'Активные вакансии',
      newOffers: 'Новые вакансии',
      medianSalary: 'Медиана',
      p90Salary: 'P90',
      offersWithSalaryRange: 'Вилки',
      remoteWorkModeSplit: 'Удалённо',
      hybridWorkModeSplit: 'Гибридно',
      officeWorkModeSplit: 'Стационарно',
      permanentEmployment: 'ТК',
      vsPreviousPeriod: 'vs. предыдущие 7 дней',
    },
    cv: {
      title: 'CV',
      load: 'Загрузить',
      processing: 'Обработка…',
      preview: 'Просмотр',
    },
    salaryDistribution: {
      title: 'Распределение зарплат',
      brackets: {
        below10k: 'Ниже 10k',
        from10to15k: '10k – 15k',
        from15to20k: '15k – 20k',
        from20to25k: '20k – 25k',
        from25to30k: '25k – 30k',
        from30to35k: '30k – 35k',
        above35k: 'Выше 35k',
      },
    },
    popularTechnologies: {
      title: 'Навыки',
      searchLabel: 'Поиск технологий',
      searchPlaceholder: 'Поиск…',
      clearSearch: 'Очистить поиск',
      editExperience: 'Редактировать опыт',
      experienceLevelLabel: 'Мой опыт',
      experienceLevelPlaceholder: 'Выберите уровень',
      experienceComment: 'Комментарий',
      experienceLevel: {
        master: 'Master',
        regular: 'Regular',
        adept: 'Adept',
        'knew-before': 'Когда-то знал',
        'to-learn': 'Хочу изучить',
        'not-interested': 'Не интересует',
      },
      columns: {
        rank: '#',
        technology: 'Технология',
        offers: 'Вакансии',
        share: 'Доля',
        median: 'Медиана',
        experience: 'Я',
      },
    },
  },
  energyMeter: {
    title: 'Учёт энергии',
    description: 'Отслеживайте потребление энергии и стоимость в реальном времени',
    status: 'Статус',
    energyTariff: 'Тариф',
    meterStatus: {
      reset: 'Сброшен',
      started: 'Измерение',
      stopped: 'Остановлен',
    },
    stats: {
      consumedEnergy: 'Потреблено',
      totalCost: 'Стоимость',
      avgPower: 'Ср. мощность',
    },
    currentPower: {
      label: 'Мощность',
      subtractBaseline: 'Вычитать базовую мощность',
    },
    timer: {
      label: 'Таймер',
      modeAriaLabel: 'Режим таймера',
      noLimit: 'Без лимита',
      withTimer: 'С таймером',
      targetTime: 'Целевое время',
    },
    duration: {
      label: 'Длительность',
      units: 'ч : мин : сек',
      start: 'СТАРТ',
      stop: 'СТОП',
      reset: 'Сброс',
    },
    timeUnit: {
      hours: 'ч',
      minutes: 'мин',
      seconds: 'сек',
      increase: 'Увеличить',
      decrease: 'Уменьшить',
    },
  },
  dashboard: {
    title: 'Главная',
    description: 'Сводка самой важной информации о доме.',
    common: {
      emptyMessage: 'Пока здесь ничего нового',
      editPreferences: 'Редактировать настройки',
      copy: 'Копировать',
    },
    hintExplanations: {
      energyHighDraw: {
        line1: 'Мгновенная мощность составляет {value} Вт.',
      },
      hotBedroom: {
        line1: 'Температура в спальне составляет {value}°C.',
      },
      coldBedroom: {
        line1: 'Температура в спальне составляет {value}°C.',
      },
      strongWind: {
        line1: 'Скорость ветра составляет {value} м/с.',
      },
      hotOutdoor: {
        line1: 'Температура на улице составляет {value}°C.',
      },
      highUv: {
        line1: 'Индекс УФ составляет {value}.',
      },
      frost: {
        line1: 'Температура на улице составляет {value}°C.',
      },
      poorOutdoorAir: {
        line1: 'AQI на улице составляет {value}.',
      },
      hourlyRain: {
        line1: 'Вероятность осадков достигает {value}%.',
      },
      hourlySnow: {
        line1: 'Вероятность осадков достигает {value}%.',
      },
      hourlyHail: {
        line1: 'Вероятность осадков достигает {value}%.',
      },
      hourlySleet: {
        line1: 'Вероятность осадков достигает {value}%.',
      },
      hourlyIce: {
        line1: 'Вероятность осадков достигает {value}%.',
      },
      hourlyMixed: {
        line1: 'Вероятность осадков достигает {value}%.',
      },
      hourlyHotOutdoor: {
        line1: 'В ближайшие часы температура достигнет {value}°C.',
      },
      hourlyStrongWind: {
        line1: 'В ближайшие часы ветер достигнет {value} м/с.',
      },
      hourlyHighUv: {
        line1: 'В ближайшие часы индекс УФ достигнет {value}.',
      },
      hourlyFrost: {
        line1: 'В ближайшие часы температура опустится до {value}°C.',
      },
      ventilate: {
        highCo2: 'CO₂ высокий — проветривание улучшит качество воздуха.',
        elevatedCo2: 'Повышенный CO₂ — рассмотрите краткое проветривание.',
        highHumidity: 'Высокая влажность в доме, а на улице воздух суше.',
      },
    },
    energy: {
      title: 'Энергия',
      consumptionToday: 'Сегодня',
      avgDailyConsumption: 'Ср. день',
      grossPrice: 'С НДС',
      fixedFee: 'Абонплата',
      reading: 'Показания',
      cost: 'Стоим.',
      avgMonthlyConsumption: 'Ср. месяц',
      avgMonthlyCost: 'Стоим. мес',
      instantDraw: 'Нагрузка',
      highDraw: 'Высокое потребление',
    },
    indoor: {
      title: 'Качество воздуха',
      co2Reading: 'CO₂',
      humidityReading: 'Влажность',
      airQuality: 'AQI',
      dusk: 'Закат',
      dawn: 'Рассвет',
      ventilate: {
        highCo2: 'Стоит проветрить — высокий CO₂',
        elevatedCo2: 'Рассмотрите проветривание — повышенный CO₂',
        highHumidity: 'Рассмотрите проветривание — высокая влажность',
        tooCold: 'На улице слишком холодно для проветривания',
        tooHot: 'На улице слишком жарко для проветривания',
        poorOutdoorAir: 'Плохое качество воздуха на улице',
        tooWindy: 'Слишком ветрено для проветривания',
        comfortable: 'Воздух в норме',
        missingData: 'Недостаточно данных для оценки проветривания',
      },
    },
    temperature: {
      title: 'Температура',
      bathroomFloor: 'Пол в ванной',
      livingRoom: 'Гостиная',
      bedroom: 'Спальня',
      bathroom: 'Ванная',
      hotBedroom: 'Высокая температура в спальне',
      coldBedroom: 'Низкая температура в спальне',
    },
    weather: {
      title: 'Погода',
      temperature: 'Температура',
      uvIndex: 'Индекс УФ',
      humidity: 'Влажность',
      windSpeed: 'Скорость ветра',
      windGusts: 'Порывы ветра',
      sunAltitude: 'Высота солнца',
      moonAltitude: 'Высота луны',
      strongWind: 'Сильный ветер',
      hotOutdoor: 'Высокая температура на улице',
      highUv: 'Высокий индекс УФ',
      frost: 'Мороз',
      poorOutdoorAir: 'Плохое качество воздуха на улице',
      beaufortScale: [
        'Штиль',
        'Тихий ветер',
        'Лёгкий ветер',
        'Слабый ветер',
        'Умеренный ветер',
        'Свежий ветер',
        'Сильный ветер',
        'Крепкий ветер',
        'Шторм',
        'Сильный шторм',
        'Жестокий шторм',
        'Ураган',
        'Ураганный ветер',
      ],
    },
    weatherForecast: {
      title: 'Прогноз погоды',
    },
    hourlyWeatherForecast: {
      title: 'Почасовой прогноз',
      rainChance: 'Вероятность дождя в ближайшие часы',
      snowChance: 'Вероятность снега в ближайшие часы',
      hailChance: 'Вероятность града в ближайшие часы',
      sleetChance: 'Вероятность мокрого снега в ближайшие часы',
      iceChance: 'Вероятность ледяного дождя в ближайшие часы',
      mixedChance: 'Вероятность смешанных осадков в ближайшие часы',
    },
    jobAds: {
      title: 'Вакансии',
      applied: 'Отклик отправлен',
      favourite: 'Избранное',
      removeFromFavourites: 'Удалить из избранного',
      addToFavourites: 'Добавить в избранное',
      markAsApplied: 'Отметить как откликнутую',
      editApplication: 'Редактировать отклик',
      company: 'Компания',
      currentApplicationStatus: 'Текущий статус',
      applicationDate: 'Дата отклика',
      rejectionDate: 'Дата отказа',
      requiredSkills: 'Требуемые навыки',
      changeApplicationStatus: 'Изменить статус',
      checkCvMatch: 'Проверить соответствие',
      cvMatchUnavailableTheProtocol: 'Сопоставление недоступно',
      cvMatchUnavailableTheProtocolTooltip: 'Вакансия с The Protocol — анализ CV пока не поддерживается',
      cvMatchUnavailableTheProtocolTitle: 'Анализ соответствия недоступен',
      cvMatchUnavailableTheProtocolNotice:
        'Эта вакансия с портала The Protocol. Пока нельзя получить полное описание объявления, поэтому сопоставление с CV недоступно.',
      viewMatchAnalysis: 'Показать анализ соответствия CV',
      matchAnalysisTitle: 'Анализ соответствия CV',
      matchAnalysisScore: '{score}%',
      matchAnalysisStaleTooltip: 'Анализ относится к более ранней версии CV',
      matchAnalysisStaleNotice: 'Анализ относится к более ранней версии CV — выполните анализ заново.',
      matchAnalysisSummarySection: 'Резюме',
      matchAnalysisStrengthsSection: 'Сильные стороны',
      matchAnalysisGapsSection: 'Пробелы',
      matchAnalysisMustHaveGapsSection: 'Недостающие обязательные требования',
      matchAnalysisMustHaveGapsEmpty: 'нет',
      matchAnalysisMustHaveGapsIndicatorLabel: '{count} недостающих обязательных требований',
      matchAnalysisMustHaveGapsClearLabel: 'Нет недостающих обязательных требований',
      matchAnalysisObservationsSection: 'Наблюдения',
      matchAnalysisConclusionSection: 'Выводы',
      newApplicationStatus: 'Новый статус',
      newArchiveReason: 'Причина архивации',
      applicationComment: 'Комментарий',
      save: 'Сохранить',
      cancel: 'Отмена',
      publishedTodayTag: 'новая',
      filters: {
        label: 'Фильтр',
        skillsLabel: 'Навыки',
        appliedOnlyLabel: 'Показать только откликнутые',
        appliedOnlyActiveLabel: 'Показать все архивные',
      },
      acceptableSalary: {
        label: 'Минимальная зарплата',
        ariaLabel: 'Минимально приемлемая зарплата',
        valueAtLeast: '{amount} k PLN',
      },
      workplaceType: {
        office: 'Офис',
        remote: 'Удалённо',
        hybrid: 'Гибрид',
      },
      applyStatus: {
        'pending-review': 'На просмотр',
        consider: 'На рассмотрении',
        applied: 'Отклик отправлен',
        interview: 'Собеседование',
        archived: 'В архиве',
      },
      archiveReason: {
        other: 'Другое',
        'company-excluded': 'Исключённая компания',
        'unmet-requirements': 'Невыполненные требования',
        'stack-mismatch': 'Не мой стек',
        'weak-match': 'Слабое соответствие',
        'manager-track': 'Менеджерский трек',
        'no-response': 'Нет ответа',
        rejected: 'Отказ',
        withdrawn: 'Отозван',
        'offer-accepted': 'Оффер принят',
      },
      addManualJobAd: 'Добавить объявление',
      addManualJobAdTitle: 'Добавить объявление вручную',
      addManualJobAdUrl: 'URL объявления',
      addManualJobAdSubmit: 'Добавить',
      jobTitle: 'Название',
      employmentType: {
        label: 'Тип договора',
        permanent: 'Трудовой договор',
        b2b: 'B2B',
        uod: 'UoD',
        mandate_contract: 'Zlecenie',
        any: 'UoP / B2B',
        contract: 'Контракт',
        internship: 'Стажировка',
        intern: 'Стажировка',
      },
      workplaceTypeLabel: 'Формат работы',
      salaryHintPermanent: 'Суммы брутто в месяц (PLN)',
      salaryHintB2b: 'Суммы нетто в месяц (PLN)',
      paidVacationDaysLabel: 'Оплачиваемый отпуск (дни)',
      paidVacationDaysHint: 'Пусто = без коррекции отпуска. Указанные дни повышают почасовую ставку.',
      salaryFrom: 'Зарплата от',
      salaryTo: 'Зарплата до',
      editManualJobAd: 'Уточнить детали',
      editManualJobAdTitle: 'Уточнить детали объявления',
      editManualJobAdSubmit: 'Сохранить',
      deleteManualJobAd: 'Удалить',
      deleteManualJobAdTitle: 'Удалить объявление?',
      deleteManualJobAdConfirm: 'Вы уверены, что хотите навсегда удалить объявление «{title}»?',
      deleteManualJobAdHint:
        'При ошибке в URL, названии или компании удалите объявление и добавьте заново (доступно 24 часа).',
    },
    news: {
      title: 'Новости',
      markAsUnread: 'Отметить как непрочитанное',
      markAsRead: 'Отметить как прочитанное',
    },
    stockMarket: {
      title: 'Биржа',
      symbol: 'Символ',
      earnings: 'До отчёта',
      pe: 'P/E',
      toPriceTarget: 'До цели',
      priceTarget: 'Цель',
      peAtTarget: 'P/E@Цель',
      quote: 'Котировка',
      change: 'Изменение',
      priceTargetChange: 'Изменение цели',
      status: {
        afterHours: 'После закрытия',
        preMarket: 'Предторговый период',
        closed: 'Закрыта',
        open: 'Открыта',
      },
      openingIn: 'Открытие через',
      closingIn: 'Закрытие через',
      durationHourSuffix: 'ч',
    },
    torrents: {
      title: 'Торренты',
      search: 'Поиск',
      showAll: 'Показать все',
      searchTorrents: 'Поиск торрентов',
      clearSearch: 'Очистить поиск',
      download: 'Скачать',
      searchFor: 'Искать торренты для',
    },
  },
  offline: {
    title: 'Нет подключения',
    description: 'Похоже, вы offline. Проверьте сеть и попробуйте снова.',
    retry: 'Обновить',
  },
  notFound: {
    title: 'Страница не найдена',
    description: 'Адрес, который вы ищете, не соответствует ни одной странице приложения.',
    backHome: 'На главную',
  },
  error: {
    title: 'Что-то пошло не так',
    description: 'В приложении произошла непредвиденная ошибка. Попробуйте ещё раз.',
    retry: 'Попробовать снова',
    unexpected: 'Непредвиденная ошибка',
  },
  dateTime: {
    todayShort: 'Сегодня',
    tomorrowShort: 'Завтра',
    weekdayShort: ['пн.', 'вт.', 'ср.', 'чт.', 'пт.', 'сб.', 'вс.'],
    weekdayLong: ['понедельник', 'вторник', 'среда', 'четверг', 'пятница', 'суббота', 'воскресенье'],
  },
}
