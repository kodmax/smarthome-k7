...
describe('sortLowForwardPETickers', () => {
  it('excludes tickers without forward PE at price target', () => {
    const result = sortLowForwardPETickers(
      stockMarketFeed(
  ...
  it('includes tickers with forward PE at price target equal to 25', () => {
    const result = sortLowForwardPETickers(
      stockMarketFeed(ticker({ symbol: 'EDGE', price: { priceTarget: 150 }, statistics: { forwardEPS: 6 } })).tickers,
    )

    expect(calcPEAtPT(result[0]!).forwardPEAtPriceTarget).toBe(25)
    expect(result.map(item => item.symbol)).toEqual(['EDGE'])
  })
  ...
  it('sorts tickers by ascending forward-to-trailing PE factor at price target', () => {
    ...
    expect(result.map(item => item.symbol)).toEqual(['LOWER_FACTOR', 'HIGHER_FACTOR'])
  })
})
