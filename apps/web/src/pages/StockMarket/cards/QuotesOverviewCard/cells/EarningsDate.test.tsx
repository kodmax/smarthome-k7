import { screen } from '@testing-library/react'
import { afterEach, beforeEach, describe, expect, it, vi } from 'vitest'
import { ticker } from '@/pages/Dashboard/test/fixtures/stockMarket'
import { renderInTableRow } from '@/pages/Dashboard/test/renderInTable'
import { EarningsDate } from './EarningsDate'

describe('EarningsDate', () => {
  beforeEach(() => {
    vi.useFakeTimers()
    vi.setSystemTime(new Date('2026-06-28T12:00:00'))
  })

  afterEach(() => {
    vi.useRealTimers()
  })

  it('shows days left for an upcoming earnings date within 30 days', () => {
    renderInTableRow(<EarningsDate ticker={ticker({ symbol: 'AAA', earningsDate: { confirmed: '2026-07-15' } })} />)

    expect(screen.getByText(/17d/)).toBeInTheDocument()
  })

  it('renders empty cell when earnings date is outside 30 days', () => {
    renderInTableRow(<EarningsDate ticker={ticker({ symbol: 'AAA', earningsDate: { confirmed: '2026-08-15' } })} />)

    expect(screen.getByRole('cell')).toBeEmptyDOMElement()
  })
})
