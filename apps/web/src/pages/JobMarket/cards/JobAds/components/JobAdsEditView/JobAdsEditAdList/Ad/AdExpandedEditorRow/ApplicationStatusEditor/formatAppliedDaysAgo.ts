...
export function formatApplicationTooltip(
  ...
  const lines =
    appliedAt !== null
      ? [formatAppliedDaysAgo(appliedAt, locale, now), ...contentLines.filter(line => line.length > 0)]
      : contentLines.filter(line => line.length > 0)
