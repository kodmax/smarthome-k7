...
export const source: DataSourceDefinition<FuelPrices> = {
    ...
    script: async () => {
        return await myFetch('https://www.autocentrum.pl/paliwa/ceny-paliw/mazowieckie/')
            .then(content => new JSDOM(content.toString('utf8')).window.document).then(async (document: Document) => {
                const current = Object.fromEntries(Array.from(document.querySelectorAll('.fuels-wrapper .station-detail-wrapper')).map(price => {
                    return [getTextContent(price, '.fuel-header').trim(), +getTextContent(price, '.price').trim().replace(/\s*zł$/, '').replace(',', '.')]
                }))
                ...
                try {
                    for (const type of Object.keys(current)) {
                        await conn.query(
                            'insert into commodities (datetime, name, price) values (?, ?, ?)',
                            [now, type, current[type]]
                        )
