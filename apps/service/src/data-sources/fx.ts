...
export const source: DataSourceDefinition<FX> = {
    ...
    script: async () => {
        const [eur, usd, chf, gbp, uah, rub] = await Promise.all([
            ...
            myFetch('https://pl.investing.com/currencies/eur-pln', { accept: 'text/html' }).then(response => response.toString('utf-8')).then(html => {
                return getTextContent(new JSDOM(html).window.document.body, '.text-2xl[data-test=instrument-price-last]')
            }),

            myFetch('https://pl.investing.com/currencies/usd-pln', { accept: 'text/html' }).then(response => response.toString('utf-8')).then(html => {
                return getTextContent(new JSDOM(html).window.document.body, '.text-2xl[data-test=instrument-price-last]')
            }),

            myFetch('https://pl.investing.com/currencies/chf-pln', { accept: 'text/html' }).then(response => response.toString('utf-8')).then(html => {
                return getTextContent(new JSDOM(html).window.document.body, '.text-2xl[data-test=instrument-price-last]')
            }),

            myFetch('https://pl.investing.com/currencies/gbp-pln', { accept: 'text/html' }).then(response => response.toString('utf-8')).then(html => {
                return getTextContent(new JSDOM(html).window.document.body, '.text-2xl[data-test=instrument-price-last]')
            }),

            myFetch('https://pl.investing.com/currencies/pln-uah', { accept: 'text/html' }).then(response => response.toString('utf-8')).then(html => {
                return getTextContent(new JSDOM(html).window.document.body, '.text-2xl[data-test=instrument-price-last]')
            }),

            myFetch('https://pl.investing.com/currencies/pln-rub', { accept: 'text/html' }).then(response => response.toString('utf-8')).then(html => {
                return getTextContent(new JSDOM(html).window.document.body, '.text-2xl[data-test=instrument-price-last]')
            })
        ])
        ...
        try {
            for (const pair of Object.keys(rates)) {
                await conn.query(
                    'insert into fx (datetime, currency_pair, exchange_rate) values (?, ?, ?)',
                    [now, pair, rates[pair as keyof FXRates]]
                )
