...
export const source: DataSourceDefinition<Torrent[]> = {
    ...
    script: async () => {
        return await myFetch('https://apibay.org/precompiled/data_top100_207.json').then(async resp => JSON.parse(resp.toString('utf-8')))
    },

    push: (push, command) => {
        command.on('search', args => {
            myFetch(`https://apibay.org/q.php?q=${encodeURIComponent(args)}&cat=207`)
                .then(async resp => {
