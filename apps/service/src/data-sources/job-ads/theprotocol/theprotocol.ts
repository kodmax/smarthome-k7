...
export const fetchTheprotocolOffers = async () => {
  const req = await observeHttpRequest(
    theprotocolAdsUrl,
    () =>
      fetch(theprotocolAdsUrl, {
        headers: {
          ...
          cookie: config.jobAds.theprotocolCookie,
          Referer: 'https://theprotocol.it/filtry/javascript;t/1;s/warszawa;wp/zdalna,hybrydowa,stacjonarna;rw',
        },
        body: '{"typesOfContractIds":[],"positionLevelIds":[],"salaryFrom":"1","cities":["Warszawa"],"workModeCodes":["home-office","hybrid","full-office"],"onlyWithProjectDescription":false,"expectedTechnologies":["JavaScript"],"niceToHaveTechnologies":[],"excludedTechnologies":[],"regionsOfWorld":[],"keywords":[],"specializationsCodes":[],"isSupportingUkraine":false,"fromExternalLocations":true}',
        method: 'POST',
      }),
    'json',
  )
