...
const jsDev: (skills: Offer['skills']) => boolean = skills => {
  return skills.some(skill => skill.name.startsWith('React'))
}
...
const isFullyRemote: (offer: Offer) => boolean = offer => offer.remote || offer.workplace_type === 'remote'
  ...
  return myFetch('https://justjoin.it/api/offers', { accept: 'application/json' })
    ...
    .then(async (offers: Offer[]) => {
      ...
      for (const top of jobs.slice(0, Math.floor(0.2 * jobs.length))) {
        const details = await myFetch('https://justjoin.it/api/offers/' + top.id).then(
          resp => JSON.parse(resp.toString('utf-8')) as Details,
        )
