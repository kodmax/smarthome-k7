...
export const source: DataSourceDefinition<JobsOffers> = {
    ...
    script: async () => {
        ...
        try {
            ...
            await conn.query('insert into commodities (datetime, name, price) values (?, ?, ?)', [now, 'salaries:median/kPLN', salaries.median])
            await conn.query('insert into commodities (datetime, name, price) values (?, ?, ?)', [now, 'salaries:top20/kPLN', salaries.top20])
            ...
            const myList = offers.filter(offer =>
