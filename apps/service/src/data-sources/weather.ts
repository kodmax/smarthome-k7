...
const windDirectionCodes = ['N', 'NNE', 'NE', 'ENE', 'E', 'ESE', 'SE', 'SSE', 'S', 'SSW', 'SW', 'WSW', 'W', 'WNW', 'NW', 'NNW']
    ...
    script: async () => {
        ...
        const [forecast, instant, allergens, hourly, aq] = await Promise.all([
            myFetch('https://www.accuweather.com/pl/pl/warsaw/274663/10-day-weather-forecast/274663')
                .then(content => new JSDOM(content.toString('utf-8')).window.document).then((document: Document) => {
                    const forecast = []
            ...
            myFetch('https://www.accuweather.com/pl/pl/warsaw/274663/current-weather/274663')
                .then(content => new JSDOM(content.toString('utf-8')).window.document).then((document: Document) => {
                    ...
                    const details: Record<string, string> = Object.fromEntries(
                        Array.from(document.querySelectorAll('.current-weather-card .current-weather-details .detail-item'))
                            .map(detail => ([detail.children[0].textContent, detail.children[1].textContent]))
                    )
            ...
            myFetch('https://www.accuweather.com/pl/pl/warsaw/274663/weather-forecast/274663')
                .then(content => new JSDOM(content.toString('utf-8')).window.document).then((document: Document) => {
                    return Array.from(document.querySelectorAll('.allergy-forecast .allergy') as NodeListOf<HTMLAnchorElement>).slice(0, 5).map(allergen => {
                        return {
            ...
            myFetch('https://www.accuweather.com/pl/pl/warsaw/274663/hourly-weather-forecast/274663')
                .then(content => new JSDOM(content.toString('utf-8')).window.document).then((document: Document) => {
                    return Array.from(document.querySelectorAll('.hourly-wrapper .hour')).map(hour => {
                        return {
                            precipIcon: basename(new URL(hour.querySelector('.precip img.precip-icon')?.getAttribute('data-src') ?? '').pathname),
                        ...
                        return {
                            sun: { altitude: sunPosition.altitude / Math.PI * 180, azimuth: sunPosition.azimuth / Math.PI * 180 },
                            ...forecast
                        }
                    })
                }),
            ...
            myFetch('https://www.accuweather.com/pl/pl/warsaw/274663/air-quality-index/274663')
                .then(content => new JSDOM(content.toString('utf-8')).window.document).then((document: Document) => {
                    ...
                    const pollutants = Object.fromEntries(Array.from(document.querySelectorAll('#pollutants .air-quality-pollutant')).map(pollutant => {
                        return [
        ...
        try {
            await conn.query('insert into outdoor_temp (datetime, value) values (?, ?)', [datetime.getDateTime(), instant.temp])
            await conn.query('insert into pressure (datetime, pressure) values (?, ?)', [datetime.getDateTime(), instant.pressure])
