...
Server.listen({}, async apollo => {
    ...
    await KnxLink.connect('192.168.1.8', { events: knxEvents }).then(async knx => {
        ...
        const heatersReadings = {
            bathroomState: knxSwitch('home.heating.lazienka.water-heating', knx.getDatapoint(heating['Grzejniki wodne']['Lazienka stan'])),
            bathroomFloorState: knxSwitch('home.heating.lazienka.floor-heating', knx.getDatapoint(heating['Podłoga Łazienka'].state)),
            livingRoomState: knxSwitch('home.heating.salon.water-heating', knx.getDatapoint(heating['Grzejniki wodne']['Salon stan'])),
            bedroomState: knxSwitch('home.heating.sypialnia.water-heating', knx.getDatapoint(heating['Grzejniki wodne']['Sypialnia stan'])),
        }
        const hvacModes = {
            livingroomMode: KnxHVACMode('home.heating.hvacmode.living-room', knx.getDatapoint(({ DataType: DPT_HVACMode, address: '2/0/4' }))),
            bathroomMode: KnxHVACMode('home.heating.hvacmode.living-room', knx.getDatapoint(({ DataType: DPT_HVACMode, address: '2/1/4' }))),
            bedroomMode: KnxHVACMode('home.heating.hvacmode.living-room', knx.getDatapoint(({ DataType: DPT_HVACMode, address: '2/2/4' })))
        }
        ...
        feeds.addFeed('energy', { energyCost, energyConsumption, ...energyReadings }, ({ energyCost, total, instant, energyConsumption, meter }) => ({
        ...
        const co2Alert = knxSwitch('home.air-quality.co2-alert', knx.getDatapoint({ address: '2/5/1', DataType: DPT_Alarm }))
        ...
        feeds.addFeed('home.air-quality.humidity', { humidityReading: knxHumidity('home.air-quality.humidity', knx.getDatapoint(airQuality.Wilgotność.reading)) })

        feeds.addFeed('home.temp.bathroom-floor', {
        ...
        feeds.addFeed('home.temp.bedroom', {
        ...
        feeds.addFeed('home.temp.livingroom', {
        ...
        feeds.addFeed('home.temp.bathroom', {
