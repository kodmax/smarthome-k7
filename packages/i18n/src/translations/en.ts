import { type Translations } from './types'

export const en: Translations = {
  nav: {
    dashboard: 'Dashboard',
    stockMarket: 'Stock Market',
    jobMarket: 'Job Market',
    energyMeter: 'Energy Meter',
    appearance: 'Appearance',
    about: 'About',
    settings: 'Settings',
  },
  appearance: {
    title: 'Appearance',
    description: 'Application appearance settings.',
    theme: 'Theme',
    themeAriaLabel: 'Application theme',
    system: 'System',
    systemAriaLabel: 'System theme',
    light: 'Light',
    lightAriaLabel: 'Light theme',
    dark: 'Dark',
    darkAriaLabel: 'Dark theme',
    language: 'Language',
    languageAriaLabel: 'Application language',
    dashboard: 'Dashboard',
    dashboardJobAdsSalary: 'Show salary in job ads on the dashboard',
    sentryTest: 'Sentry',
    sentryTestDescription: 'Send a test error to the frontend and backend to verify source maps in Sentry.',
    sentryTestButton: 'Send test error',
    sentryTestSent: 'Test errors sent to Sentry (web and service).',
    sentryTestUnavailable: 'Sentry is only available in production with a configured DSN.',
  },
  stockMarket: {
    title: 'Stock Market',
    description: 'Stock quotes and company analysis',
    marketStatus: {
      title: 'Market session',
    },
    marketIndices: {
      sp500: {
        title: 'S&P 500',
      },
      sp500Futures: {
        title: 'Futures',
      },
    },
    forex: {
      usdPln: {
        title: 'USD/PLN rate',
      },
    },
    earningsTomorrow: {
      title: 'Earnings tomorrow',
    },
    highUpside: {
      title: 'High upside',
    },
    lowForwardPE: {
      title: 'Low forward P/E at target',
    },
    earningsSoon: {
      title: 'Quarterly report soon',
    },
    priceTargetChange: {
      title: 'Target change',
    },
    quotesOverview: {
      title: 'Overview',
    },
    views: {
      label: 'View',
    },
  },
  stockQuotes: {
    title: 'Quotes',
  },
  jobMarket: {
    title: 'Job Market',
    description: 'JavaScript / TypeScript job market analysis',
    summary: {
      activeOffers: 'Active listings',
      newOffers: 'New listings',
      medianSalary: 'Median',
      p90Salary: 'P90',
      offersWithSalaryRange: 'Ranges',
      remoteWorkModeSplit: 'Remote',
      hybridWorkModeSplit: 'Hybrid',
      officeWorkModeSplit: 'On-site',
      permanentEmployment: 'Permanent',
      vsPreviousPeriod: 'vs. previous 7 days',
    },
    cv: {
      title: 'CV',
      load: 'Load',
      processing: 'Processing…',
      preview: 'Preview',
    },
    salaryDistribution: {
      title: 'Salary distribution',
      brackets: {
        below10k: 'Below 10k',
        from10to15k: '10k – 15k',
        from15to20k: '15k – 20k',
        from20to25k: '20k – 25k',
        from25to30k: '25k – 30k',
        from30to35k: '30k – 35k',
        above35k: 'Above 35k',
      },
    },
    popularTechnologies: {
      title: 'Skills',
      searchLabel: 'Search technologies',
      searchPlaceholder: 'Search…',
      clearSearch: 'Clear search',
      editExperience: 'Edit experience',
      experienceLevelLabel: 'My experience',
      experienceLevelPlaceholder: 'Select level',
      experienceComment: 'Comment',
      experienceLevel: {
        master: 'Master',
        regular: 'Regular',
        adept: 'Adept',
        'knew-before': 'Used to know it',
        'to-learn': 'Want to learn',
        'not-interested': 'Not interested',
      },
      columns: {
        rank: '#',
        technology: 'Technology',
        offers: 'Listings',
        share: 'Share',
        median: 'Median',
        experience: 'Me',
      },
    },
  },
  energyMeter: {
    title: 'Energy Meter',
    description: 'Monitor energy usage and cost in real time',
    status: 'Status',
    energyTariff: 'Energy tariff',
    meterStatus: {
      reset: 'Reset',
      started: 'Measuring',
      stopped: 'Stopped',
    },
    stats: {
      consumedEnergy: 'Energy used',
      totalCost: 'Total cost',
      avgPower: 'Avg power',
    },
    currentPower: {
      label: 'Current power',
      subtractBaseline: 'Subtract baseline power from result',
    },
    timer: {
      label: 'Timer',
      modeAriaLabel: 'Timer mode',
      noLimit: 'No limit',
      withTimer: 'With timer',
      targetTime: 'Target time',
    },
    duration: {
      label: 'Duration',
      units: 'hr : min : sec',
      start: 'START',
      stop: 'STOP',
      reset: 'Reset',
    },
    timeUnit: {
      hours: 'hr',
      minutes: 'min',
      seconds: 'sec',
      increase: 'Increase',
      decrease: 'Decrease',
    },
  },
  dashboard: {
    title: 'Dashboard',
    description: 'Summary of the most important information from your home.',
    common: {
      emptyMessage: 'Nothing new here yet',
      editPreferences: 'Edit preferences',
      copy: 'Copy',
    },
    hintExplanations: {
      energyHighDraw: {
        line1: 'Instant power draw is {value} W.',
      },
      hotBedroom: {
        line1: 'Bedroom temperature is {value}°C.',
      },
      coldBedroom: {
        line1: 'Bedroom temperature is {value}°C.',
      },
      strongWind: {
        line1: 'Wind speed is {value} m/s.',
      },
      hotOutdoor: {
        line1: 'Outdoor temperature is {value}°C.',
      },
      highUv: {
        line1: 'UV index is {value}.',
      },
      frost: {
        line1: 'Outdoor temperature is {value}°C.',
      },
      poorOutdoorAir: {
        line1: 'Outdoor AQI is {value}.',
      },
      hourlyRain: {
        line1: 'Precipitation chance reaches up to {value}%.',
      },
      hourlySnow: {
        line1: 'Precipitation chance reaches up to {value}%.',
      },
      hourlyHail: {
        line1: 'Precipitation chance reaches up to {value}%.',
      },
      hourlySleet: {
        line1: 'Precipitation chance reaches up to {value}%.',
      },
      hourlyIce: {
        line1: 'Precipitation chance reaches up to {value}%.',
      },
      hourlyMixed: {
        line1: 'Precipitation chance reaches up to {value}%.',
      },
      hourlyHotOutdoor: {
        line1: 'Temperature in the next few hours will reach {value}°C.',
      },
      hourlyStrongWind: {
        line1: 'Wind in the next few hours will reach {value} m/s.',
      },
      hourlyHighUv: {
        line1: 'UV index in the next few hours will reach {value}.',
      },
      hourlyFrost: {
        line1: 'Temperature in the next few hours will drop to {value}°C.',
      },
      ventilate: {
        highCo2: 'CO₂ is high — ventilating will improve air quality.',
        elevatedCo2: 'CO₂ is elevated — consider a short ventilation.',
        highHumidity: 'Indoor humidity is high and outdoor air is drier.',
      },
    },
    energy: {
      title: 'Energy',
      consumptionToday: 'Today',
      avgDailyConsumption: 'Daily avg.',
      grossPrice: 'Gross price',
      fixedFee: 'Fixed fee',
      reading: 'Reading',
      cost: 'Cost',
      avgMonthlyConsumption: 'Monthly avg.',
      avgMonthlyCost: 'Monthly cost',
      instantDraw: 'Live draw',
      highDraw: 'High power draw',
    },
    indoor: {
      title: 'Air quality',
      co2Reading: 'CO₂',
      humidityReading: 'Humidity',
      airQuality: 'AQI',
      dusk: 'Dusk',
      dawn: 'Dawn',
      ventilate: {
        highCo2: 'Ventilate — CO₂ is high',
        elevatedCo2: 'Consider ventilating — CO₂ is elevated',
        highHumidity: 'Consider ventilating — humidity is high',
        tooCold: 'Too cold outside to ventilate',
        tooHot: 'Too hot outside to ventilate',
        poorOutdoorAir: 'Poor outdoor air quality',
        tooWindy: 'Too windy to ventilate',
        comfortable: 'Air quality is comfortable',
        missingData: 'Not enough data to assess ventilation',
      },
    },
    temperature: {
      title: 'Temperature',
      bathroomFloor: 'Bathroom floor',
      livingRoom: 'Living room',
      bedroom: 'Bedroom',
      bathroom: 'Bathroom',
      hotBedroom: 'High bedroom temperature',
      coldBedroom: 'Low bedroom temperature',
    },
    weather: {
      title: 'Weather',
      temperature: 'Temperature',
      uvIndex: 'UV index',
      humidity: 'Humidity',
      windSpeed: 'Wind speed',
      windGusts: 'Wind gusts',
      sunAltitude: 'Sun altitude',
      moonAltitude: 'Moon altitude',
      strongWind: 'Strong wind',
      hotOutdoor: 'High outdoor temperature',
      highUv: 'High UV index',
      frost: 'Frost',
      poorOutdoorAir: 'Poor outdoor air quality',
      beaufortScale: [
        'Calm',
        'Light air',
        'Light breeze',
        'Gentle breeze',
        'Moderate breeze',
        'Fresh breeze',
        'Strong breeze',
        'Near gale',
        'Gale',
        'Strong gale',
        'Storm',
        'Violent storm',
        'Hurricane',
      ],
    },
    weatherForecast: {
      title: 'Weather forecast',
    },
    hourlyWeatherForecast: {
      title: 'Hourly forecast',
      rainChance: 'Rain chance in the next hours',
      snowChance: 'Snow chance in the next hours',
      hailChance: 'Hail chance in the next hours',
      sleetChance: 'Sleet chance in the next hours',
      iceChance: 'Freezing rain chance in the next hours',
      mixedChance: 'Mixed precipitation chance in the next hours',
    },
    jobAds: {
      title: 'Job offers',
      applied: 'Applied',
      favourite: 'Favourite',
      removeFromFavourites: 'Remove from favourites',
      addToFavourites: 'Add to favourites',
      markAsApplied: 'Mark as applied',
      editApplication: 'Edit application',
      company: 'Company',
      currentApplicationStatus: 'Current status',
      applicationDate: 'Application date',
      rejectionDate: 'Rejection date',
      requiredSkills: 'Required skills',
      changeApplicationStatus: 'Change status',
      checkCvMatch: 'Check match',
      cvMatchUnavailableTheProtocol: 'Match unavailable',
      cvMatchUnavailableTheProtocolTooltip: 'The Protocol listing — CV match is not supported yet',
      cvMatchUnavailableTheProtocolTitle: 'CV match unavailable',
      cvMatchUnavailableTheProtocolNotice:
        'This listing is from The Protocol. We cannot fetch the full job description yet, so CV matching is not available.',
      viewMatchAnalysis: 'Show CV match analysis',
      matchAnalysisTitle: 'CV match analysis',
      matchAnalysisScore: '{score}%',
      matchAnalysisStaleTooltip: 'Analysis is based on an older CV version',
      matchAnalysisStaleNotice: 'This analysis is based on an older CV version — re-analyze to refresh it.',
      matchAnalysisSummarySection: 'Summary',
      matchAnalysisStrengthsSection: 'Strengths',
      matchAnalysisGapsSection: 'Gaps',
      matchAnalysisMustHaveGapsSection: 'Must-have gaps',
      matchAnalysisMustHaveGapsEmpty: 'none',
      matchAnalysisMustHaveGapsIndicatorLabel: '{count} missing must-have requirements',
      matchAnalysisMustHaveGapsClearLabel: 'No must-have gaps',
      matchAnalysisObservationsSection: 'Observations',
      matchAnalysisConclusionSection: 'Conclusion',
      newApplicationStatus: 'New status',
      newArchiveReason: 'Archive reason',
      applicationComment: 'Comment',
      save: 'Save',
      cancel: 'Cancel',
      publishedTodayTag: 'new',
      filters: {
        label: 'Filter',
        skillsLabel: 'Skills',
        appliedOnlyLabel: 'Show applied only',
        appliedOnlyActiveLabel: 'Show all archived',
      },
      acceptableSalary: {
        label: 'Minimum salary',
        ariaLabel: 'Minimum acceptable salary',
        valueAtLeast: '{amount} k PLN',
      },
      workplaceType: {
        office: 'Office',
        remote: 'Remote',
        hybrid: 'Hybrid',
      },
      applyStatus: {
        'pending-review': 'To review',
        consider: 'To consider',
        applied: 'Applied',
        interview: 'Interview',
        archived: 'Archived',
      },
      archiveReason: {
        other: 'Other',
        'company-excluded': 'Excluded company',
        'unmet-requirements': 'Unmet requirements',
        'stack-mismatch': 'Stack mismatch',
        'weak-match': 'Weak match',
        'manager-track': 'Management track',
        'no-response': 'No response',
        rejected: 'Rejected',
        withdrawn: 'Withdrawn',
        'offer-accepted': 'Offer accepted',
      },
      addManualJobAd: 'Add listing',
      addManualJobAdTitle: 'Add listing manually',
      addManualJobAdUrl: 'Listing URL',
      addManualJobAdSubmit: 'Add',
      jobTitle: 'Title',
      employmentType: {
        label: 'Employment type',
        permanent: 'Employment contract',
        b2b: 'B2B',
        uod: 'Contract of mandate',
        mandate_contract: 'Specific-task contract',
        any: 'Employment contract / B2B',
        contract: 'Contract',
        internship: 'Internship',
        intern: 'Internship',
      },
      workplaceTypeLabel: 'Work mode',
      salaryHintPermanent: 'Gross monthly amounts (PLN)',
      salaryHintB2b: 'Net monthly amounts (PLN)',
      paidVacationDaysLabel: 'Paid vacation (days)',
      paidVacationDaysHint: 'Empty = no vacation adjustment. Entered days raise the hourly rate.',
      salaryFrom: 'Salary from',
      salaryTo: 'Salary to',
      editManualJobAd: 'Refine details',
      editManualJobAdTitle: 'Refine listing details',
      editManualJobAdSubmit: 'Save',
      deleteManualJobAd: 'Delete',
      deleteManualJobAdTitle: 'Delete listing?',
      deleteManualJobAdConfirm: 'Are you sure you want to permanently delete "{title}"?',
      deleteManualJobAdHint:
        'If the URL, title, or company is wrong, delete and re-add the listing (available for 24 hours).',
    },
    news: {
      title: 'News',
      markAsUnread: 'Mark as unread',
      markAsRead: 'Mark as read',
    },
    stockMarket: {
      title: 'Stock market',
      symbol: 'Symbol',
      earnings: 'To earnings',
      pe: 'P/E',
      toPriceTarget: 'To target',
      priceTarget: 'Target',
      peAtTarget: 'P/E@Target',
      quote: 'Quote',
      change: 'Change',
      priceTargetChange: 'Target change',
      status: {
        afterHours: 'After hours',
        preMarket: 'Pre-market',
        closed: 'Closed',
        open: 'Open',
      },
      openingIn: 'Opening in',
      closingIn: 'Closing in',
      durationHourSuffix: 'h',
    },
    torrents: {
      title: 'Torrents',
      search: 'Search',
      showAll: 'Show all',
      searchTorrents: 'Search torrents',
      clearSearch: 'Clear search',
      download: 'Download',
      searchFor: 'Search torrents for',
    },
  },
  offline: {
    title: 'You are offline',
    description: 'It looks like you lost your connection. Check your network and try again.',
    retry: 'Refresh',
  },
  notFound: {
    title: 'Page not found',
    description: 'The address you are looking for does not match any page in the app.',
    backHome: 'Back to dashboard',
  },
  error: {
    title: 'Something went wrong',
    description: 'An unexpected error occurred in the app. You can try again.',
    retry: 'Try again',
    unexpected: 'Unexpected error',
  },
  dateTime: {
    todayShort: 'Today',
    tomorrowShort: 'Tomorrow',
    weekdayShort: ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'],
    weekdayLong: ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'],
  },
}
