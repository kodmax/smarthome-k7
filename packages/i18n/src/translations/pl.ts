import { type Translations } from './types'

export const pl: Translations = {
  nav: {
    dashboard: 'Pulpit główny',
    stockMarket: 'Giełda',
    jobMarket: 'Rynek Pracy',
    energyMeter: 'Pomiar energii',
    appearance: 'Wygląd',
    about: 'O aplikacji',
    settings: 'Ustawienia',
  },
  appearance: {
    title: 'Wygląd',
    description: 'Ustawienia wyglądu aplikacji.',
    theme: 'Motyw',
    themeAriaLabel: 'Motyw aplikacji',
    system: 'Systemowy',
    systemAriaLabel: 'Motyw systemowy',
    light: 'Jasny',
    lightAriaLabel: 'Jasny motyw',
    dark: 'Ciemny',
    darkAriaLabel: 'Ciemny motyw',
    language: 'Język',
    languageAriaLabel: 'Język aplikacji',
    dashboard: 'Dashboard',
    dashboardJobAdsSalary: 'Pokaż wynagrodzenie w ofertach pracy na dashboardzie',
    sentryTest: 'Sentry',
    sentryTestDescription: 'Wyślij testowy błąd na frontend i backend, żeby sprawdzić source mapy w Sentry.',
    sentryTestButton: 'Wyślij testowy błąd',
    sentryTestSent: 'Testowe błędy wysłane do Sentry (web i service).',
    sentryTestUnavailable: 'Sentry jest dostępne tylko w produkcji z skonfigurowanym DSN.',
  },
  stockMarket: {
    title: 'Giełda',
    description: 'Notowania giełdowe i analiza spółek',
    marketStatus: {
      title: 'Sesja rynku',
    },
    marketIndices: {
      sp500: {
        title: 'S&P 500',
      },
      sp500Futures: {
        title: 'Futures',
      },
    },
    forex: {
      usdPln: {
        title: 'Kurs USD/PLN',
      },
    },
    earningsTomorrow: {
      title: 'Raport kwartalny jutro',
    },
    highUpside: {
      title: 'Duży upside',
    },
    lowForwardPE: {
      title: 'Niskie prognozowane C/Z na celu',
    },
    earningsSoon: {
      title: 'Raport kwartalny wkrótce',
    },
    priceTargetChange: {
      title: 'Zmiana celu',
    },
    quotesOverview: {
      title: 'Przegląd',
    },
    views: {
      label: 'Widok',
    },
  },
  stockQuotes: {
    title: 'Notowania',
  },
  jobMarket: {
    title: 'Rynek Pracy',
    description: 'Analiza rynku ofert pracy - JavaScript / TypeScript',
    summary: {
      activeOffers: 'Aktywne oferty',
      newOffers: 'Nowe oferty',
      medianSalary: 'Mediana',
      p90Salary: 'P90',
      offersWithSalaryRange: 'Widelki',
      remoteWorkModeSplit: 'Zdalnie',
      hybridWorkModeSplit: 'Hybrydowo',
      officeWorkModeSplit: 'Stacjonarnie',
      permanentEmployment: 'UoP',
      vsPreviousPeriod: 'vs. poprzednie 7 dni',
    },
    cv: {
      title: 'CV',
      load: 'Wczytaj',
      processing: 'Przetwarzanie…',
      preview: 'Podgląd',
    },
    salaryDistribution: {
      title: 'Rozkład wynagrodzeń',
      brackets: {
        below10k: 'Poniżej 10k',
        from10to15k: '10k – 15k',
        from15to20k: '15k – 20k',
        from20to25k: '20k – 25k',
        from25to30k: '25k – 30k',
        from30to35k: '30k – 35k',
        above35k: 'Powyżej 35k',
      },
    },
    popularTechnologies: {
      title: 'Umiejętności',
      searchLabel: 'Szukaj technologii',
      searchPlaceholder: 'Szukaj…',
      clearSearch: 'Wyczyść wyszukiwanie',
      editExperience: 'Edytuj doświadczenie',
      experienceLevelLabel: 'Moje doświadczenie',
      experienceLevelPlaceholder: 'Wybierz poziom',
      experienceComment: 'Komentarz',
      experienceLevel: {
        master: 'Master',
        regular: 'Regular',
        adept: 'Adept',
        'knew-before': 'Znałem kiedyś',
        'to-learn': 'Chcę się nauczyć',
        'not-interested': 'Nie interesuje mnie',
      },
      columns: {
        rank: '#',
        technology: 'Technologia',
        offers: 'Oferty',
        share: 'Udział',
        median: 'Mediana',
        experience: 'Ja',
      },
    },
  },
  energyMeter: {
    title: 'Pomiar energii',
    description: 'Monitoruj zużycie energii i koszt w czasie rzeczywistym',
    status: 'Status',
    energyTariff: 'Taryfa energii',
    meterStatus: {
      reset: 'Wyzerowany',
      started: 'Pomiar',
      stopped: 'Zatrzymany',
    },
    stats: {
      consumedEnergy: 'Zużyta energia',
      totalCost: 'Łączny koszt',
      avgPower: 'Średnia moc',
    },
    currentPower: {
      label: 'Aktualna moc',
      subtractBaseline: 'Odliczaj moc bazową od wyniku',
    },
    timer: {
      label: 'Timer',
      modeAriaLabel: 'Tryb timera',
      noLimit: 'Bez limitu',
      withTimer: 'Z timerem',
      targetTime: 'Czas docelowy',
    },
    duration: {
      label: 'Czas trwania',
      units: 'godz : min : sek',
      start: 'START',
      stop: 'STOP',
      reset: 'Zeruj',
    },
    timeUnit: {
      hours: 'godz',
      minutes: 'min',
      seconds: 'sek',
      increase: 'Zwiększ',
      decrease: 'Zmniejsz',
    },
  },
  dashboard: {
    title: 'Pulpit główny',
    description: 'Podsumowanie najważniejszych informacji z domu.',
    common: {
      emptyMessage: 'Na razie nie ma tu nic nowego',
      editPreferences: 'Edytuj preferencje',
      copy: 'Kopiuj',
    },
    hintExplanations: {
      energyHighDraw: {
        line1: 'Chwilowy pobór wynosi {value} W.',
      },
      hotBedroom: {
        line1: 'Temperatura w sypialni wynosi {value}°C.',
      },
      coldBedroom: {
        line1: 'Temperatura w sypialni wynosi {value}°C.',
      },
      strongWind: {
        line1: 'Prędkość wiatru wynosi {value} m/s.',
      },
      hotOutdoor: {
        line1: 'Temperatura na zewnątrz wynosi {value}°C.',
      },
      highUv: {
        line1: 'Indeks UV wynosi {value}.',
      },
      frost: {
        line1: 'Temperatura na zewnątrz wynosi {value}°C.',
      },
      poorOutdoorAir: {
        line1: 'AQI na zewnątrz wynosi {value}.',
      },
      hourlyRain: {
        line1: 'Prawdopodobieństwo opadów wynosi do {value}%.',
      },
      hourlySnow: {
        line1: 'Prawdopodobieństwo opadów wynosi do {value}%.',
      },
      hourlyHail: {
        line1: 'Prawdopodobieństwo opadów wynosi do {value}%.',
      },
      hourlySleet: {
        line1: 'Prawdopodobieństwo opadów wynosi do {value}%.',
      },
      hourlyIce: {
        line1: 'Prawdopodobieństwo opadów wynosi do {value}%.',
      },
      hourlyMixed: {
        line1: 'Prawdopodobieństwo opadów wynosi do {value}%.',
      },
      hourlyHotOutdoor: {
        line1: 'Temperatura w najbliższych godzinach sięgnie {value}°C.',
      },
      hourlyStrongWind: {
        line1: 'Wiatr w najbliższych godzinach sięgnie {value} m/s.',
      },
      hourlyHighUv: {
        line1: 'Indeks UV w najbliższych godzinach sięgnie {value}.',
      },
      hourlyFrost: {
        line1: 'Temperatura w najbliższych godzinach spadnie do {value}°C.',
      },
      ventilate: {
        highCo2: 'CO₂ jest wysokie — przewietrzenie poprawi jakość powietrza.',
        elevatedCo2: 'CO₂ jest podwyższone — rozważ krótkie przewietrzenie.',
        highHumidity: 'Wilgotność w domu jest wysoka, a na dworze powietrze jest suchsze.',
      },
    },
    energy: {
      title: 'Energia',
      consumptionToday: 'Zużycie dziś',
      avgDailyConsumption: 'Śr. zużycie dzienne',
      grossPrice: 'Cena brutto',
      fixedFee: 'Opłata stała',
      reading: 'Pomiar',
      cost: 'Koszt',
      avgMonthlyConsumption: 'Śr. zużycie msc',
      avgMonthlyCost: 'Śr. koszt za msc',
      instantDraw: 'Chwilowy pobór',
      highDraw: 'Wysoki pobór mocy',
    },
    indoor: {
      title: 'Jakość powietrza',
      co2Reading: 'CO₂',
      humidityReading: 'Wilgotność',
      airQuality: 'AQI',
      dusk: 'Zmierzch',
      dawn: 'Świt',
      ventilate: {
        highCo2: 'Warto przewietrzyć — wysokie CO₂',
        elevatedCo2: 'Rozważ przewietrzenie — podwyższone CO₂',
        highHumidity: 'Rozważ przewietrzenie — wysoka wilgotność',
        tooCold: 'Na dworze za zimno na przewietrzenie',
        tooHot: 'Na dworze za ciepło na przewietrzenie',
        poorOutdoorAir: 'Na dworze słaba jakość powietrza',
        tooWindy: 'Za silny wiatr na przewietrzenie',
        comfortable: 'Powietrze w normie',
        missingData: 'Brak danych do oceny przewietrzenia',
      },
    },
    temperature: {
      title: 'Temperatura',
      bathroomFloor: 'Podłoga łazienki',
      livingRoom: 'Salon',
      bedroom: 'Sypialnia',
      bathroom: 'Łazienka',
      hotBedroom: 'Wysoka temperatura w sypialni',
      coldBedroom: 'Niska temperatura w sypialni',
    },
    weather: {
      title: 'Pogoda',
      temperature: 'Temperatura',
      uvIndex: 'Indeks UV',
      humidity: 'Wilgotność',
      windSpeed: 'Predkość wiatru',
      windGusts: 'Porywy wiatru',
      sunAltitude: 'Wysokość słońca',
      moonAltitude: 'Wysokość księżyca',
      strongWind: 'Silny wiatr',
      hotOutdoor: 'Wysoka temperatura na zewnątrz',
      highUv: 'Wysoki indeks UV',
      frost: 'Mróz',
      poorOutdoorAir: 'Słaba jakość powietrza na zewnątrz',
      beaufortScale: [
        'Cisza, flauta',
        'Powiew',
        'Słaby wiatr',
        'Łagodny wiatr',
        'Umiarkowany wiatr',
        'Dość silny wiatr',
        'Silny wiatr',
        'Bardzo silny wiatr',
        'Sztorm, wicher',
        'Silny sztorm',
        'Bardzo silny sztorm',
        'Gwałtowny sztorm',
        'Huragan',
      ],
    },
    weatherForecast: {
      title: 'Prognoza pogody',
    },
    hourlyWeatherForecast: {
      title: 'Prognoza godzinowa',
      rainChance: 'Szansa na deszcz w najbliższych godzinach',
      snowChance: 'Szansa na śnieg w najbliższych godzinach',
      hailChance: 'Szansa na grad w najbliższych godzinach',
      sleetChance: 'Szansa na deszcz ze śniegiem w najbliższych godzinach',
      iceChance: 'Szansa na marznący deszcz w najbliższych godzinach',
      mixedChance: 'Szansa na mieszane opady w najbliższych godzinach',
    },
    jobAds: {
      title: 'Oferty pracy',
      applied: 'Złożone',
      favourite: 'Ulubione',
      removeFromFavourites: 'Usuń z ulubionych',
      addToFavourites: 'Dodaj do ulubionych',
      markAsApplied: 'Oznacz jako złożone',
      editApplication: 'Edytuj aplikację',
      company: 'Firma',
      currentApplicationStatus: 'Obecny status',
      applicationDate: 'Data zaaplikowania',
      rejectionDate: 'Data odrzucenia',
      requiredSkills: 'Wymagane umiejętności',
      changeApplicationStatus: 'Zmień stan',
      checkCvMatch: 'Sprawdź dopasowanie',
      cvMatchUnavailableTheProtocol: 'Dopasowanie niedostępne',
      cvMatchUnavailableTheProtocolTooltip: 'Oferta z The Protocol — analiza CV nie jest jeszcze obsługiwana',
      cvMatchUnavailableTheProtocolTitle: 'Analiza dopasowania niedostępna',
      cvMatchUnavailableTheProtocolNotice:
        'Ta oferta pochodzi z portalu The Protocol. Na razie nie da się pobrać pełnego opisu ogłoszenia, więc porównanie z CV nie jest dostępne.',
      viewMatchAnalysis: 'Pokaż analizę dopasowania CV',
      matchAnalysisTitle: 'Analiza dopasowania CV',
      matchAnalysisScore: '{score}%',
      matchAnalysisStaleTooltip: 'Analiza dotyczy wcześniejszej wersji CV',
      matchAnalysisStaleNotice: 'Analiza dotyczy wcześniejszej wersji CV — przeanalizuj ponownie.',
      matchAnalysisSummarySection: 'Podsumowanie',
      matchAnalysisStrengthsSection: 'Mocne strony',
      matchAnalysisGapsSection: 'Luki',
      matchAnalysisMustHaveGapsSection: 'Brakujące wymagania must-have',
      matchAnalysisMustHaveGapsEmpty: 'brak',
      matchAnalysisMustHaveGapsIndicatorLabel: '{count} brakujących wymagań must-have',
      matchAnalysisMustHaveGapsClearLabel: 'Brak luk must-have',
      matchAnalysisObservationsSection: 'Obserwacje',
      matchAnalysisConclusionSection: 'Wnioski',
      newApplicationStatus: 'Nowy status',
      newArchiveReason: 'Powód archiwizacji',
      applicationComment: 'Komentarz',
      save: 'Zapisz',
      cancel: 'Anuluj',
      publishedTodayTag: 'nowa',
      filters: {
        label: 'Filtr',
        skillsLabel: 'Umiejętności',
        appliedOnlyLabel: 'Pokaż tylko zaaplikowane',
        appliedOnlyActiveLabel: 'Pokaż wszystkie zarchiwizowane',
      },
      acceptableSalary: {
        label: 'Minimalne wynagrodzenie',
        ariaLabel: 'Minimalne akceptowalne wynagrodzenie',
        valueAtLeast: '{amount} k PLN',
      },
      workplaceType: {
        office: 'Biuro',
        remote: 'Zdalnie',
        hybrid: 'Hybrydowo',
      },
      applyStatus: {
        'pending-review': 'Do przejrzenia',
        consider: 'Do rozważenia',
        applied: 'Zaaplikowane',
        interview: 'Rozmowa',
        archived: 'Zarchiwizowane',
      },
      archiveReason: {
        other: 'Inny',
        'company-excluded': 'Wykluczona firma',
        'unmet-requirements': 'Niespełnione wymagania',
        'stack-mismatch': 'Poza stackiem',
        'weak-match': 'Słabe dopasowanie',
        'manager-track': 'Ścieżka managerska',
        'no-response': 'Brak odpowiedzi',
        rejected: 'Odrzucone',
        withdrawn: 'Rozmyśliłem się',
        'offer-accepted': 'Oferta zaakceptowana',
      },
      addManualJobAd: 'Dodaj ogłoszenie',
      addManualJobAdTitle: 'Dodaj ogłoszenie ręcznie',
      addManualJobAdUrl: 'URL ogłoszenia',
      addManualJobAdSubmit: 'Dodaj',
      jobTitle: 'Tytuł',
      employmentType: {
        label: 'Typ umowy',
        permanent: 'UoP',
        b2b: 'B2B',
        uod: 'UoD',
        mandate_contract: 'Zlecenie',
        any: 'UoP / B2B',
        contract: 'Kontrakt',
        internship: 'Staż',
        intern: 'Staż',
      },
      workplaceTypeLabel: 'Tryb pracy',
      salaryHintPermanent: 'Kwoty brutto miesięcznie (PLN)',
      salaryHintB2b: 'Kwoty netto miesięcznie (PLN)',
      paidVacationDaysLabel: 'Płatny urlop (dni)',
      paidVacationDaysHint: 'Puste = bez korekty urlopu. Podane dni podwyższają stawkę godzinową.',
      salaryFrom: 'Wynagrodzenie od',
      salaryTo: 'Wynagrodzenie do',
      editManualJobAd: 'Doprecyzuj szczegóły',
      editManualJobAdTitle: 'Doprecyzuj szczegóły ogłoszenia',
      editManualJobAdSubmit: 'Zapisz',
      deleteManualJobAd: 'Usuń',
      deleteManualJobAdTitle: 'Usunąć ogłoszenie?',
      deleteManualJobAdConfirm: 'Czy na pewno chcesz trwale usunąć ogłoszenie „{title}"?',
      deleteManualJobAdHint:
        'Przy błędnym URL, tytule lub firmie usuń ogłoszenie i dodaj je ponownie (dostępne przez 24 h).',
    },
    news: {
      title: 'Wiadomości',
      markAsUnread: 'Oznacz jako nieprzeczytane',
      markAsRead: 'Oznacz jako przeczytane',
    },
    stockMarket: {
      title: 'Giełda',
      symbol: 'Symbol',
      earnings: 'Do wyników',
      pe: 'C/Z',
      toPriceTarget: 'Do celu',
      priceTarget: 'Cel',
      peAtTarget: 'C/Z@Cel',
      quote: 'Notowanie',
      change: 'Zmiana',
      priceTargetChange: 'Zmiana celu',
      status: {
        afterHours: 'Po zamknięciu',
        preMarket: 'Przed otwarciem',
        closed: 'Zamknięta',
        open: 'Otwarta',
      },
      openingIn: 'Otwarcie za',
      closingIn: 'Zamknięcie za',
      durationHourSuffix: 'g',
    },
    torrents: {
      title: 'Torrenty',
      search: 'Szukaj',
      showAll: 'Pokaż wszystkie',
      searchTorrents: 'Szukaj torrentów',
      clearSearch: 'Wyczyść wyszukiwanie',
      download: 'Pobierz',
      searchFor: 'Szukaj torrentów dla',
    },
  },
  offline: {
    title: 'Brak połączenia',
    description: 'Wygląda na to, że jesteś offline. Sprawdź sieć i spróbuj ponownie.',
    retry: 'Odśwież',
  },
  notFound: {
    title: 'Strona nie istnieje',
    description: 'Adres, którego szukasz, nie prowadzi do żadnej strony w aplikacji.',
    backHome: 'Wróć na pulpit',
  },
  error: {
    title: 'Coś poszło nie tak',
    description: 'W aplikacji wystąpił nieoczekiwany błąd. Możesz spróbować ponownie.',
    retry: 'Spróbuj ponownie',
    unexpected: 'Nieoczekiwany błąd',
  },
  dateTime: {
    todayShort: 'Dziś',
    tomorrowShort: 'Jutro',
    weekdayShort: ['pon.', 'wt.', 'śr.', 'czw.', 'pt.', 'sob.', 'niedz.'],
    weekdayLong: ['poniedziałek', 'wtorek', 'środa', 'czwartek', 'piątek', 'sobota', 'niedziela'],
  },
}
