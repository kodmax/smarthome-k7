...
KnxLink.connect(requireEnv("KNX_HOST"), { maxRetry: 3 }).then(async (knx) => {
  ...
  try {
    ...
    await db.query(
      "insert into hourly_energy_readings (datetime, hour, hour_start_reading) values (?, ?, ?)",
      [
    ...
    await db.query(
      "update hourly_energy_readings set hourly_consumption = ? - hour_start_reading where datetime = ?",
      [total.value, prevHour.toISOString().substring(0, 19)],
    );
  } finally {
