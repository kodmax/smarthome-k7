#!/usr/bin/node
import { DPT_Value_Humidity, KnxLink, DPT_Value_Temp, DPT_State, DPT_Value_AirQuality } from 'js-knx'
...
KnxLink.connect(requireEnv("KNX_HOST"), { maxRetry: 3 }).then(async knx => {
    ...
    try {
        ...
        await db.query(`insert into indoor_air_condition (
            bathroom_floor_temp, 
            ...
            bathroomFloor.temp.value, bathroomFloor.state.value,
            bedroom.temp.value, bedroom.state.value, bedroom.setpoint.value,
            livingroom.temp.value, livingroom.state.value, livingroom.setpoint.value,
            bathroom.temp.value, bathroom.state.value, bathroom.setpoint.value,
            dewPoint.value,
            humidity.value,
            co2.value
        ])
