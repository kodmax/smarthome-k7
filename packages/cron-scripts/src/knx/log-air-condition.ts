...
KnxLink.connect(requireEnv("KNX_HOST"), { maxRetry: 3 }).then(async (knx) => {
  ...
  try {
    ...
    const humidity = await knx
      .getDatapoint({ address: "2/3/5", DataType: DPT_Value_Humidity })
      .read();
    const dewPoint = await knx
      .getDatapoint({ address: "2/3/10", DataType: DPT_Value_Temp })
      .read();
    const co2 = await knx
      .getDatapoint({ address: "2/3/3", DataType: DPT_Value_AirQuality })
      .read();
